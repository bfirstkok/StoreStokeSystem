...
ระบบจัดการคลังวัสดุสำหรับบันทึกวัสดุคงเหลือ เพิ่มวัสดุเข้าคลัง เบิกออกคลัง ดูประวัติการเคลื่อนไหว และจัดการรูปภาพวัสดุผ่าน Supabase Storage
...
- เพิ่มวัสดุใหม่
- ใส่จำนวนตั้งต้นได้ทันทีโดยไม่ต้องไปทำรายการรับเข้าแยก
- นำเข้าวัสดุหลายรายการจาก Excel
- แก้ไขวัสดุ
- ลบวัสดุ
- อัปโหลดรูปวัสดุ
- เลือกหมวดหมู่วัสดุ
...
### เพิ่มวัสดุ/เอาออกคลัง

- หน้า `คลังวัสดุ` สำหรับเพิ่มวัสดุใหม่พร้อมจำนวนตั้งต้น
...
หลังเพิ่มวัสดุแล้วไม่ต้องไปทำรายการ `ขอเข้าคลัง` ซ้ำ เพราะจำนวนที่ใส่ไว้ถูกบันทึกเป็นจำนวนในคลังแล้ว
...
```text
เพิ่มวัสดุในหน้าคลัง -> ของออกคลังเมื่อต้องเบิก -> ดูประวัติ -> ดู dashboard
```
## Shared Account Users

ระบบรองรับการใช้ 1 บัญชีล็อกอินร่วมกันหลายคน เช่น บัญชีร้านหรือบัญชีครอบครัวเดียวกัน แล้วเลือกชื่อผู้ใช้งานย่อยหลังล็อกอิน

Flow การใช้งาน:

1. ล็อกอินด้วยบัญชีหลัก
2. ระบบพาไปหน้า `/select-user`
3. เลือกชื่อผู้ใช้งานย่อย หรือเพิ่มชื่อใหม่
4. หลังเลือกแล้วจึงเข้า Dashboard
5. รายการรับเข้า/เอาออกคลังจะบันทึกชื่อผู้ใช้งานย่อยเป็นผู้ทำรายการ

ข้อมูลคลังยังแยกตามบัญชีหลักเหมือนเดิม ดังนั้นบัญชีล็อกอินคนละบัญชีจะไม่เห็นคลังของกันและกัน แต่ผู้ใช้งานย่อยภายใต้บัญชีหลักเดียวกันจะใช้คลังร่วมกัน

ต้องรัน SQL นี้ใน Supabase ก่อนใช้ฟีเจอร์:

```text
account_users_setup.sql
```

ไฟล์นี้จะสร้าง/อัปเดต:

- ตาราง `public.account_users`
- คอลัมน์ `stock_movements.account_user_id`
- index สำหรับโหลดรายชื่อผู้ใช้และประวัติ
- RLS policy ให้แต่ละบัญชีหลักจัดการเฉพาะผู้ใช้งานย่อยของตัวเอง

## Stock Out Search And Filters

หน้า `/stock-out` มีช่องค้นหาและตัวกรองสำหรับหา/เบิกวัสดุออกจากคลัง:

- ค้นหาจากชื่อวัสดุหรือหมวดหมู่
- กรองตามหมวดหมู่
- กรองตามสถานะสต็อก: ทุกสถานะ, พร้อมเบิก, ใกล้หมด, หมดสต็อก
- ระบบยังคงป้องกันการเบิกออกเกินจำนวนคงเหลือ
